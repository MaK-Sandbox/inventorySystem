...
import { getItems, getLocations, createItem, updateItem, removeItem } from './api';
...
function diffItem(prev: Item, next: ItemFormPayload): Partial<ItemFormPayload> {
  ...
  if (next.location_id !== prev.location_id) patch.location_id = next.location_id;
  if (next.purchase_price !== prev.purchase_price) patch.purchase_price = next.purchase_price;
  if (next.purchase_date !== prev.purchase_date) patch.purchase_date = next.purchase_date;
  if ((next.freeText || null) !== (prev.freeText || null)) patch.freeText = next.freeText;
  return patch;
}
...
export default function App() {
  ...
  useEffect(() => {
    Promise.all([getItems(), getLocations()])
      ...
      .catch(() => showToast('Failed to load data. Is the server running?', 'error'));
  ...
  const visibleItems = useMemo(() => {
    ...
    return items
      .filter(i => activeLocationId == null || i.location_id === activeLocationId)
      .filter(i => !q || i.name.toLowerCase().includes(q) || (i.freeText ?? '').toLowerCase().includes(q))
      .sort((a, b) => b.id - a.id);
  }, [items, activeLocationId, searchQuery]);

  const topbarTitle = activeLocationId != null
  ...
  return (
    <>
      <Sidebar
        ...
        onSelectLocation={id => { setActiveLocationId(id); setSearchQuery(''); }}
      />
