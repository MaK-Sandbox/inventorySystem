...
router.post("/", (req, res) => {
    ...
    const insertStatement = db.prepare(`INSERT INTO locations (${validatedProperties.join(", ")}) VALUES (?, ?, ?)`);
    ...
    res.json(db.prepare("SELECT * FROM locations WHERE id = ?").get(newLocationId));
});
router.put("/:id", (req, res) => {
    ...
    for (const key in req.body) {
        if (Object.prototype.hasOwnProperty.call(req.body, key)) {
            ...
            const updateStatement = db.prepare(`UPDATE locations SET ${key} = ? WHERE id = ?`);
