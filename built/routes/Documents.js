...
function docDir() {
    ...
    if (!dir)
        throw new Error("DOC_DIR environment variable is not set");
    return dir;
}
...
router.post("/item/:itemId", async (req, res) => {
    ...
    const uploaded = Array.isArray(req.files.files)
        ? req.files.files
        : [req.files.files];
    ...
    const insert = db.prepare("INSERT INTO documents (item_id, path, description) VALUES (?, ?, ?)");
    const created = [];
